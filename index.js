...
/**
 * @swagger
 * /user-id-by-hash/{hash}:
 *   get:
 *     summary: Get user ID by user hash
 *     parameters:
 *       - in: path
 *         name: hash
 *         required: true
 ...
 */
app.get("/user-id-by-hash/:hash", async (req, res) => {
	const { hash } = req.params;

	try {
		const [user] = await db.query("SELECT id FROM users WHERE hash = ?", [hash]);
		...
		console.error("Error fetching user by hash:", err);
...
app.delete("/food_logs/:id", authenticate, async (req, res) => {
	await db.query("DELETE FROM food_logs WHERE id = ?", [req.params.id]);
 ...
 *                 example: {"goal":2500,"theme":"system","height":null,"weight":null,"autocalculate":false,"lang":"ru"}
 *     responses:
 *       200:
...
app.post("/update-settings", authenticate, async (req, res) => {
    ...
    try {
        ...
        // Convert settings object to a stringified JSON
        const settingsString = JSON.stringify(settings);

        // Update the user's settings in the database
        await db.query("UPDATE users SET settings = ? WHERE id = ?", [settingsString, userId]);
